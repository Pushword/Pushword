...
class MediaExtension
{
  public function __construct(
    ...
    #[Autowire('%pw.public_media_dir%')] private readonly string $publicMediaDir,
  ) {}
  ...
  public function renderImage(
    ...
    if (null !== $wrapper || null !== $wrapperClass) {
      ...
      $classAttr =
        null !== $wrapperClass
          ? sprintf(' class="%s"', htmlspecialchars($wrapperClass))
          : '';
  ...
  #[AsTwigFunction('imageUrl')]
  public function getImageUrl(
    Media|string $media,
    string $filterName = 'default',
    ?string $extension = null,
  ): string {
    $mediaObj = $media instanceof Media ? $media : $this->transformStringToMedia($media);

    return $this->imageCacheManager->getBrowserPath($mediaObj, $filterName, $extension);
  }
  ...
  public function transformStringToMedia(
    Media|string|int|null $src,
    Media|string $name = '',
  ): Media {
    if (null === $src || '' === $src) {
      throw new Exception(
        'media_from_string() received an empty source. Check your template includes pass a valid `image`, `image_src` or `imageSrc` variable.',
      );
    }
    ...
    if (
      '' !== $name &&
      1 === preg_match('/^[a-zA-Z0-9_-]+\.(jpg|jpeg|png|gif|webp)$/i', $name) &&
    ...
    if ($this->mayBeAnInternalImage($src)) {
      $media =
        $this->findMediaByFileName($src) ??
        throw new Exception(
          "Internal - Can't handle the value submitted `" . $src . '`.',
        );
