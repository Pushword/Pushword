...
#[AsCommand(name: 'pw:page:delete', description: 'Delete every page carrying a given tag')]
final readonly class DeletePageCommand
{
    ...
    public function __invoke(
        ...
        if ('' === $tag) {
            if ($agentMode) {
                $this->writeAgentJson($output, ['tool' => 'pw:page:delete', 'result' => 'failed', 'error' => '--tag is required']);
            } else {
        ...
        if ([] === $pages) {
            if ($agentMode) {
                $this->writeAgentJson($output, ['tool' => 'pw:page:delete', 'result' => 'done', 'tag' => $tag, 'deleted' => 0, 'slugs' => []]);
            } else {
        ...
        if (! $force) {
            if (! $input->isInteractive()) {
                if ($agentMode) {
                    $this->writeAgentJson($output, ['tool' => 'pw:page:delete', 'result' => 'blocked', 'tag' => $tag, 'matched' => \count($pages), 'slugs' => $slugs, 'error' => 'pass --force to delete without a terminal']);
                } else {
            ...
            $io->listing($slugs);
            if (! $io->confirm(\sprintf('Delete %d page(s) tagged `%s`?', \count($pages), $tag), false)) {
                return Command::SUCCESS;
            }
        }
        ...
        if ($agentMode) {
            $this->writeAgentJson($output, ['tool' => 'pw:page:delete', 'result' => 'done', 'tag' => $tag, 'deleted' => \count($pages), 'slugs' => $slugs]);
        } else {
            $io->success(\sprintf('Deleted %d page(s) tagged `%s`.', \count($pages), $tag));
        }
