...
class Media implements IdInterface, Taggable, Stringable
{
    ...
    /**
     * True while the filename still tracks the reference alt — i.e. it was machine-derived
     * at upload and never manually renamed. Both `alt` and `fileName` are seeded from the
     * same original name on upload, and caption-driven renaming keeps them in sync, so they
     * diverge only when a human sets a slug independently of the alt. Used to decide whether
     * a media may still be auto-renamed from an EditorJS caption.
     *
     * The reference alt defaults to the live raw alt, but callers should pass the *persisted*
     * alt when the live one may have been mutated (e.g. MediaExtension syncs alt = caption in
     * memory while rendering), otherwise a just-changed caption makes the media look renamed.
     */
    public function isFileNameAutoGenerated(?string $referenceAlt = null): bool
    {
        if ('' === $this->fileName) {
            return false;
        }

        $base = basename($this->fileName, MediaFileName::extractExtension($this->fileName));

        return $base === MediaFileName::slugify($referenceAlt ?? $this->getAlt(true));
    }
