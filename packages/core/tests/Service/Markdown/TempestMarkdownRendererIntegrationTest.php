...
final class TempestMarkdownRendererIntegrationTest extends KernelTestCase
{
    ...
    public static function compatibleDynamicSources(): iterable
    {
        ...
        yield 'namespaced PHP class in list code' => ['- **Events:** See `Pushword\\Core\\Event\\PushwordEvents`.'];
        yield 'wrapped link label in list' => ["- The [ELTS\n  price](/pricing) increased."];
        yield 'separate code spans with braces' => ['A `collection` of `{name, type}` values.'];
        yield 'table with code braces' => ["| Key | Value |\n|---|---|\n| `source` | `{host}/{slug}` |"];
        yield 'indented closing fence' => ["```bash\n  echo hello\n  ```"];
        yield 'paragraph before a fence' => ["**Before:**\n```twig\n<div class=\"example\">\n```"];
        yield 'numbered item with a fence' => ["5. **Unlock**: Release the lock\n   ```bash\n   echo hello\n   ```"];
        yield 'fence closed with more backticks' => ["```twig\n{{ example }}\n````"];
        yield 'fenced item among numbered items' => ["1. Check spam\n2. Configure mailer\n   ```bash\n   MAILER_DSN=smtp://example.com\n   ```\n3. Check logs"];
        yield 'indented paragraph continuation' => ["A sentence\n   continued here."];
        yield 'heading containing only code' => ['### `media/index.html.twig`'];
        yield 'ordered list with child bullets' => ["1. **Step 1**: Begin\n2. **Step 2**: Choose\n   - Yes\n   - No"];
        yield 'task list with a continued item' => ["- [ ] First\n      Continued\n- [ ] Second"];
