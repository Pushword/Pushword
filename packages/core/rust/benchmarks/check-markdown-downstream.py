...
def compare(binary, cases, counts, page_equal):
    if not cases:
        return
    request = json.dumps(
        {
...
def main():
    if len(sys.argv) != 3:
        raise SystemExit("Usage: check-markdown-downstream.py <private.ndjson> <probe-binary>")
    ...
    with open(source, encoding="utf-8") as stream:
        for line in stream:
            ...
            if batch and (
                ...
                compare(binary, batch, counts, page_equal)
                batch = []
                batch_bytes = 0
            batch.append(case)
            batch_bytes += size
