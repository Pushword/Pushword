...
Astro made "ship less JavaScript" a practical default for content-driven sites. Pushword
starts from a different place: it includes the CMS and its editor. Which one fits depends
on who publishes and how the site is deployed.

The two projects share several design choices despite those different starting points.

## The short answer

Start with who needs to publish: only developers, or editors too?

**If only developers publish**, and content stays in the repository, pick Astro. Its
ecosystem is much larger than Pushword's, and its single authoring surface keeps the
workflow simple.

**If editors also publish, Pushword is the better default for a team that wants the editor built in**,
...
[Comparing like for like](#comparing-like-for-like) covers both setups.

Other reasons to consider Pushword:
...
Pushword's smaller community does not make it a new project.

Pushword's first release on Packagist predates Astro's public launch, and it has shipped
continuously since. It now has 25 packages (18 Symfony bundles) and
over 3,000 test methods. It is small in _audience_, not in age.

**The underlying stack has its own communities.** Symfony, Doctrine and Twig have extensive
...
Pushword was built for its authors' client sites and has run them since 2020. It had no
launch campaign or growth team. Features were added when those sites needed them.

That leaves a smaller community, fewer tutorials and fewer people to call for help. It also
means the roadmap follows the sites its authors maintain. Every Pushword feature described
on this page is in production, not just a demo.

## The structural difference

Both projects ship static HTML and can render dynamically. They differ in **who owns the
...
That affects four everyday tasks:
...
| Rolling back content              | Depends on content source | `git revert` and re-import, or the version extension |
...
The authoring workflow is the main difference between the projects.
...
CMS. **Storyblok** has an Astro integration and a visual editor that renders the
...
Pushword accepts content edits through:
...
When someone other than a developer needs to publish, compare **Astro plus an editing
system** with **Pushword on its own**. Astro supplies the rendering framework; Pushword
includes the CMS.

The resulting setups differ:
...
**Decap** and **TinaCMS** can store content in git and
...
Both projects support static output, Markdown workflows and optional client-side JavaScript.

| Idea                                                | Astro                                                            | Pushword                                                         |
| --------------------------------------------------- | ---------------------------------------------------------------- | ---------------------------------------------------------------- |
| Zero JavaScript by default                          | Islands architecture: static HTML with isolated interactive pockets | Server-rendered Twig; JS is opt-in per component                 |
...
Astro's `server:defer` renders a
...
Three Pushword features shipped in 2026 after its authors studied Astro's approach. The
first two follow ideas Astro shipped earlier.
...
validated at build time, with error messages and TypeScript autocomplete. Astro has
supported this since 2023.
...
The CSS API needs neither a router nor JavaScript. Its availability is a matter of
timing: the native cross-document API reached Chrome 126+ and Safari 18.2+ recently,
...
Pushword's page-cache design takes a different route from Astro's.
...
Astro fits several cases Pushword does not:

- **Your team is JavaScript-native.** Fighting a PHP stack to avoid a rebuild step is a bad
  trade. Astro uses the tools a JS team already knows.
...
- **You want a fast-moving project.** Astro ships majors quickly and the performance work is
  real: v7's Rust compiler and native Markdown pipeline cut build times 15 to 61%. That
  pace of change can be a reason to choose it.
- **You want a large, corporate-backed project.** Pushword has a smaller maintainer base,
  although it builds on Symfony rather than maintaining a framework of its own.

## Maintenance trade-offs: WordPress, Astro, Pushword

WordPress, Astro and Pushword put maintenance work in different places. That difference
matters after launch, when plugins and framework releases need attention.
...
| Therefore the risk is    | **Entropy**: nothing forces you to update     | **Churn**: frequent upgrades                  | **Concentration**: a small team, less choice |
...
the default, so previously-building invalid HTML now errors. Community integrations
...
Astro's rapid iteration brings compiler and build improvements, along with regular upgrade
work. Teams adopting it should budget for those upgrades.
...
Pushword offers far less choice than either alternative and has a much smaller community.
If a capability is missing, your team may have to build it; the cost depends on the size of
that missing piece.
...
the integrations are mature. If you would rather have the CMS built in, with the option to
