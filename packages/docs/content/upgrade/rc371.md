...
Re-add `pushword/installer` to your dependencies:

```bash
composer require pushword/installer
```

Or remove the orphaned scripts from `composer.json`:
...
Also remove `post-autoload-dump` if it only contains `Pushword\Installer\PostAutoloadDump::runPostAutoload`.
