 ...
 */
export function getCoords(elem: HTMLElement): { x1: number; y1: number; x2: number; y2: number } {
 ...
 */
export function getRelativeCoordsOfTwoElems(firstElem: HTMLElement, secondElem: HTMLElement): { fromTopBorder: number; fromLeftBorder: number; fromRightBorder: number; fromBottomBorder: number } {
 ...
 */
export function getCursorPositionRelativeToElement(elem: HTMLElement, event: MouseEvent): { width: number; height: number; x: number; y: number } {
  const rect = elem.getBoundingClientRect();
 ...
 * @returns {HTMLElement}
 */
export function insertBefore(newNode: Node, referenceNode: Node): Node {
 ...
 * @returns {void}
 */
export function focus(element: Element, atStart = true): void {
