...
/**
 * Which `<div>` is still open while markdown chunks are classified in document
 * order, so markerhood stays symmetric: a lone `</div>` chunk becomes a
 * GroupEnd only when the `<div>` it closes was imported as a GroupStart.
 *
...
export class GroupNesting {
  ...
  /** A chunk imported as a GroupStart opens a marker div. */
  openGroup(): void {
    this.openDivs.push(true)
  }

  /** Close the div a lone `</div>` chunk ends; true when a marker opened it. */
  closeGroup(): boolean {
