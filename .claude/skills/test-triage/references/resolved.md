...
## `StaticGeneratorTest::testError` serving a content page as the 404

`/fr/404.html` held a full render of "Numeric Slug YAML Test" instead of the error
template, so `assertStringContainsString('Une erreur', …)` failed on a 9 KB page.

`error.html.twig` is `{% set error_page = p('404') %}` — a real page with slug `404`
legitimately *replaces* the error template. The page came from
`PageSyncTest::testNumericSlugInFrontmatter`, which writes `numeric-slug-yaml.md` with
`slug: 404` in its frontmatter. The importer renames the file after the slug it read, so
the artifact left in the worker's content dir is `404.md` — and only
`numeric-slug-yaml.md` was tracked for deletion. The test removes its page from the DB,
so nothing looks leaked until any later import in that worker re-creates it from the
file that stayed.

Why only MariaDB and PostgreSQL went red: `StaticGeneratorTest::setUpBeforeClass()`
restores a pristine DB by copying the cached **SQLite** file over `test.db`. Against a
server database that branch cannot fire, so the class runs on whatever the worker left
behind. The SQLite jobs were not clean — they were masked.

Reproducing it needs no server database: run the class directly with a private
`TEST_RUN_ID` and list the content dir afterwards, because `.scripts/test` removes it on
exit.

```sh
TEST_RUN_ID=leakprobe vendor/bin/phpunit --filter PageSyncTest
ls /tmp/com.github.pushword.pushword/tests/leakprobe/content/localhost.dev/
```

`index.csv` also carried a `404` row; it is regenerated by the next export and did not
need cleaning. A test that imports content it wrote owes its cleanup **the name the
importer chose**, not the name it wrote.

## `LinkGraphCommandTest::testEditingAPageStalesTheGraph`

`getPublishedCorpusState()` fingerprints a host as `{pages, lastEditAt}`, and
`lastEditAt` is `MAX(updatedAt)` truncated to whole seconds. The test seeds a snapshot
stamped with the current state, edits a page, and asserts the snapshot went stale. If any
other class edited a `localhost.dev` page inside that same second, the seeded
`lastEditAt` already equals what the edit produces, the count is unchanged, and the two
states compare equal — fresh.

Nothing about it is database-specific in principle; MariaDB and PostgreSQL only widen
the window, because a `DATETIME`/`TIMESTAMP(0)` column *rounds* a fractional second where
SQLite's `Y-m-d H:i:s` string truncates it.

It reproduces in one run, deterministically, by editing any published page immediately
