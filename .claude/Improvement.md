...
**File:** `packages/core/src/Entity/Media.php` (707 lines)

Three concerns beyond data storage:

| Concern                                                                  | Extraction Target              |
| ------------------------------------------------------------------------ | ------------------------------ |
| Slug computation (`changeSlug`, `setSlugForNewMedia`, `getMediaFromFilename`)  | `MediaSlugManager` service     |
| Alt text YAML parsing (`getAltsParsed`, `editLocalizedAlt`, `updateAltSearch`) | `MediaAltManager` service      |
| Hash computation with filesystem access (`setHash`, `sha1_file`)             | Move to `MediaStorageListener` |
